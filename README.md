...
**配置文件共享：** 原版 CLI 配置 `cfdata-config.json` 现与 Web 端共用。Web 端右上角设置 → "保存当前设置" 即可持久化参数，下次启动自动加载。

**精简版 CSV 字段（11 列）：**
```
IP地址 | 端口号 | TLS | 网络延迟 | 数据中心 | 地区 | 源IP位置 | 城市 | ASN号码 | ASN组织 | 下载速度
```

**完整版 CSV（23 列）：** 精简版 + 访问协议/TLS版本/SNI/HTTP版本/WARP/Gateway/RBI/密钥交换/时间戳

**TXT 导出：** 格式 `ip:port#源IP位置-下载速度`，分隔符可自定义。

**结果操作弹窗：** 文件名默认 `ip-all`，支持"精简版"和"完整版"字段一键切换。
